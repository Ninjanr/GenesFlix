...
// Buscar os filmes favoritados do usuário
$sql_favoritos = " SELECT 
    ...
    FROM 
        favoritos
    JOIN 
        filmes ON favoritos.filme_id = filmes.id
    JOIN 
...
<body>
    ...
    <?php if (empty($filmes_favoritos)) : ?>
        <p class="paragrafo" >Você não tem nenhum filme listado!</p>
    <?php else : ?>
        <?php foreach ($filmes_favoritos as $filme) { ?>
                <div class="filme" data-id="<?= htmlspecialchars($filme['id']) ?>" data-titulo="<?= htmlspecialchars($filme['titulo']) ?>" data-descricao="<?= htmlspecialchars($filme['descricao']) ?>" data-data-lancamento="<?= htmlspecialchars($filme['ano']) ?>" data-genero="<?= htmlspecialchars($filme['genero']) ?>" data-url="<?= htmlspecialchars($filme['url']) ?>" data-media-avaliacao="<?= number_format($filme['media_avaliacao'], 2) ?>">
                    ...
                    <button  class="remove-btn" data-filme-id="<?= htmlspecialchars($filme['id']) ?>">&times;</button>
                </div>
            <?php } ?>
    <?php endif; ?>
</div>
        ...
        button.addEventListener('click', function() {
            ...
            if (confirm('Tem certeza que deseja remover este filme dos favoritos?')) {
                ...
                .then(data => {
                    if (data.success) {
                        alert(data.message);
                        this.closest('.filme').remove(); // Remove o filme da interface
                    } else {
                        alert(data.message);
                    }
                })
                .catch(error => {
                    console.error('Erro:', error);
                    alert('Ocorreu um erro ao tentar remover o filme.');
                });
            }
        });
    });
