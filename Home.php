...
$sql_filmes = "SELECT 
    ...
    GROUP_CONCAT(DISTINCT comentarios.comentario ORDER BY comentarios.data_comentario SEPARATOR ' || ') AS comentarios
FROM 
    filmes
JOIN 
...
LEFT JOIN 
    comentarios ON filmes.id = comentarios.id_filme
GROUP BY 
    filmes.id;
";
...
<head>
    ...
    <style>
        ...
        .glowing-icon {
            ...
            color: #4ade80;
            text-shadow: 0 0 10px #4ade80, 0 0 20px #4ade80, 0 0 30px #4ade80,
                0 0 40px #4ade80, 0 0 50px #4ade80;
            transition: transform 0.3s ease, text-shadow 0.3s ease;
        }

        .glowing-icon:hover {
            transform: scale(1.1);
            text-shadow: 0 0 15px #4ade80, 0 0 30px #4ade80, 0 0 45px #4ade80,
                0 0 60px #4ade80, 0 0 75px #4ade80;
        }
    </style>
</head>
...
<body>
    ...
    <div class="header">
        ...
        <nav class="navbar">
            ...
            <a href="Jogo/Tela_jogo.html"><i class="fas fa-gamepad glowing-icon"></i></a>
        </nav>
            ...
            <div class="header-right color-border">
                ...
                <div class="dropdown" id="profileDropdown">
                    ...
                    <div class="btn">
                        ...
                        <?php endif; ?>
                        <?php if ($user && in_array($user['role'], ['user', 'admin', 'admin_boss'])) : ?>
                            <a href="grafico.html" class="btn-link">Melhores Avaliados</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    <main>
        <div class="filme-container">
            ...
            <?php foreach ($imagens as $imagem) { ?>
                <div class="filme" data-id="<?= htmlspecialchars($imagem['id']) ?>" data-titulo="<?= htmlspecialchars($imagem['titulo']) ?>" data-descricao="<?= htmlspecialchars($imagem['descricao']) ?>" data-data-lancamento="<?= htmlspecialchars($imagem['ano']) ?>" data-genero="<?= htmlspecialchars($imagem['genero']) ?>" data-url="<?= htmlspecialchars($imagem['url']) ?>" data-media-avaliacao="<?= number_format($imagem['media_avaliacao'], 2) ?>" data-comentarios="<?= htmlspecialchars($imagem['comentarios']) ?>">
    ...
    <div class="modal" id="filmeModal">
        <div class="modal-content">
            ...
            <div class="comments-section">
                <h2 class="coment">Comentários:</h2>
    ...
    <div class="toggle-container">
        <div class="toggle" id="themeToggle">
            <div class="toggle-circle">
                <span class="sun"><img src="img/crescent-sun.png" alt="Sun Icon"></span>
                <span class="moon"><img src="img/crescent-moon.png" alt="Moon Icon"></span>
            </div>
        </div>
    </div>
    <br>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Adiciona o evento de clique em todos os spans com a classe 'estrela'
            document.querySelectorAll('.estrela').forEach(span => {
                span.addEventListener('click', function() {
                    const filmeId = this.getAttribute('data-id');
                    console.log(`Span clicado! ID do filme: ${filmeId}`); // Log para depuração
                    favoritarFilme(filmeId); // Chama a função para favoritar o filme
                });
            });
                    ...
                    .then(data => {
                        ...
                        if (data.length) {
                            data.forEach(comment => {
                                ...
                                commentDiv.innerHTML = `<strong>${comment.full_name}</strong>: ${comment.comentario} <br><small>${comment.data_comentario}</small>`;
